 ...
 * @note This class serves as backend controller for the speed meter element on
 * a car cockpit dashboard.
 */
class Speedometer : public QObject {
  Q_OBJECT
  /* Declare properties that can be assessed by QML elements. */
  Q_PROPERTY(
      qreal speedValue READ getSpeed WRITE setSpeed NOTIFY speedChangedSignal);
